...
String path = "/",KQ="-1", TRANG = "1",DataToSTM="";
long long last = 0;
int8_t NhipTim, Spo2 , SetupFirebase = 0, ChoPhep =0, Dung =1, Nhan =0;
...
void XuLyData()
{
  if(Dung == 1)
  {
    if(KQ !="-1")
    {
      Dung =0;
      Nhan =1;
      if(KQ.length() == 1) KQ =KQ +"B";
      Serial.println(TRANG + "A"+ KQ );
      mySerial.print(TRANG + "A"+ KQ);
    }
...
void ReadDataFromFirebase()
{
  if (Firebase.getString(firebaseData, path + "/KQ"))
  {
    KQ = firebaseData.stringData();
  }
   if (Firebase.getString(firebaseData, path + "/TRANG"))
  {
    TRANG = firebaseData.stringData();
  }
}
void WriteDataToFirebase()
{
